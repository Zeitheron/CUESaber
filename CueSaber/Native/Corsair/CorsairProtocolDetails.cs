...
namespace CUESaber.Native.Corsair
{
    ...
    internal struct CorsairProtocolDetails
    {
        ...
        {
            string sdk = Marshal.PtrToStringAuto(sdkVersion);
            string server = Marshal.PtrToStringAuto(serverVersion);

            return $"CUE SDK v{sdk}, Server: v{server}, Protocol version: SDK-{sdkProtocolVersion}/Host-{serverProtocolVersion}, Breaking changes: {breakingChanges}";
        }
    };
}
